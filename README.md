- [ ] Add Theme

- info
  - [ ] ui implementation
  - [ ] data collection
  - [ ] custom calculator
- todo
- [x] layout components for section and pages snap on screen section
- [ ] ui implementation section

  - [ ] Featured Work
  - [x] At a Glance
  - [ ] Expertise - infinite loop
  - [ ] Our method of working
  - [ ] Interior Solutions
  - [ ] What Our Clients have to say
  - [x] Our Trusted Partners
  - [ ] We are NRI Friendly Too - design
  - [ ] Footer

- [ ] Backend
  - [ ] AppScript for data collection and email
  - [ ] Server action for api call
  - [ ] Calculator
- bugs
- needs
  - [ ] Finalize ui
  - [ ] Finalize content
  - [ ] Finalize images
  - [ ] Deside seo
  - [x] Sort Hosting and domain

1. animation start margin
2. price calculator
3. interior needed to be decided to keep ? fix mobile view : chill h

- [ ] price estimator ui update (seperate section of the estimator home and internal components) - [ ]

# data used in website

testimonial (done)
working (done)
project data (pending) - needed for [expertise, featured work, projects]

- type : Healthcare | Workspace | Commercial | Institutional | Residential | Urban Design (any one)
- image : atleast one or more
- title : "this is the way"
- category: specific expertise

# we need

- contact, address and email to put on website
- about us
- User Agreement
- need a meet to decide the images for each section we are also looking

2. hero section update and finalize
3. data api layer
